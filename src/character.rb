class Character
  ...
  def file_show
    [@name.to_s, @level.to_s, @exploration.to_s]
  end
  ...
  def attack(atkr, dfndr)
    ...
    if rand(1..20) + attack_stat >= dfndr.armor
      ...
      hurt(dfndr, dam)
    elsif directions == ['back, putting distance between them', 'to the side, looking for an opening', 'forward with a feint']
  ...
  def improvements
    ...
    $prompt.select('Which blezzing you want?'.colorize(:green), list)
  end
