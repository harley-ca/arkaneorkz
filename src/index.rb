...
def main_menu
  $prompt.select('Waz iz youz doinz, choom?'.colorize(:green)) do |menu|
...
def battle
  ...
  if def_loaded == true
    ...
    $player.attack($player, dfndr) if rand(3..8) >= 4
    ...
    $player.improve if $player.exploration.remainder(5) == 0
...
def create_character(manual, named)
  if manual == -1
    ...
    while confirm_stat != 'Yes'
      system('clear')
      puts "#{name}'s Stats:"
      stats = [rand(1..6), rand(1..6), rand(1..6), rand(1..6), rand(3..10)]
      quick_stats(stats)
    ...
    $player = Character.new(name, stats[0], stats[1], stats[2], stats[3], stats[4], 11, 11, attack[0], attack[1],
                            1, 0, 20, stats[4])
    puts $player.to_s
  elsif manual == 0
...
while main_option != 'Exit'
  ...
  when 'READ ME'
    ...
    puts 'Zee you in da pit, choom!'.colorize(:green)
    next
  end
end
